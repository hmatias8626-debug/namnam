        ...
        if not items:
            ...
            for it in items:
                ...
                with st.container(border=True):
                    ...
                    if c_menos.button("➖", key=f"cart_menos_{tipo_item}_{item_id}", use_container_width=True):
                        _change_qty(tipo_item, item_id, -1)
                        st.rerun()

                    c_qty.markdown(f"""<div class="qty-num">{cantidad_item:g}</div>""", unsafe_allow_html=True)

                    if c_mas.button("➕", key=f"cart_mas_{tipo_item}_{item_id}", use_container_width=True):
                        _change_qty(tipo_item, item_id, 1)
                        st.rerun()

                    if c_borrar.button("🗑️", key=f"cart_borrar_{tipo_item}_{item_id}", use_container_width=True):
                        _remove_qty(tipo_item, item_id)
                        st.rerun()
