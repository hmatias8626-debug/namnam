...
def render():
    header("📝 Pedidos", "Pendiente → En preparación → Listo → En reparto → Entregado")
    db = require_db()
    ...
    with st.expander("➕ Crear pedido", expanded=True):
        if not productos:
            ...
            if st.button("Guardar pedido"):
                cliente_id = None
                cliente_nombre = cliente_manual.strip() or "Venta mostrador"
                if cliente_txt != "Venta mostrador / sin cliente":
                    cli = cliente_opciones[cliente_txt]
                    cliente_id = cli["id"]
