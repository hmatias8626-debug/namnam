...
def render():
    header("👥 Clientes", "Minoristas, mayoristas y saldo disponible")
    db = require_db()
    ...
    with st.expander("➕ Alta de cliente", expanded=True):
        with st.form("nuevo_cliente", clear_on_submit=True):
            ...
            if tipo_cliente == "Mayorista":
                saldo_inicial = c6.number_input("Saldo inicial / crédito cargado", min_value=0.0, step=1000.0)
            else:
            ...
            if st.form_submit_button("Guardar cliente"):
                if not nombre.strip():
                    ...
                    db.table(table("clientes")).insert({
                        ...
                        "limite_credito": 0,
                        "saldo_cuenta_corriente": saldo_inicial,
    ...
    for c in clientes:
        with st.container(border=True):
            ...
            activo = g.toggle("Activo", value=bool(c.get("activo", True)), key=f"a{c['id']}")
            ...
            if tipo_edit == "Mayorista":
                x1, x2 = st.columns(2)
                if saldo < 0:
                    x1.error(f"Debe: {money(abs(saldo))}")
                    x2.metric("Saldo disponible", money(saldo))
                else:
                    x1.success(f"Saldo disponible: {money(saldo)}")
                    x2.metric("Saldo a favor", money(saldo))
